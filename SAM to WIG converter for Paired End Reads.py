...
output_file = open('RNA-Seq of Pulldown cabRNA from phi80alpha infection of RN4220 expressing Ssc-CdnE03.wig', 'w')
...
key_bp = 1
for_strand[key_bp] = 0
# here change the value to the genome size (e.g phi80alpha-vir = 43853bp)
while key_bp != 43853:
    key_bp = int(key_bp + 1)
    for_strand[key_bp] = 0
...
with open('RNA-Seq of Pulldown cabRNA from phi80alpha infection of RN4220 expressing Ssc-CdnE03.sam') as sam_file:
    ...
    for line in sam_reader:
        ...
        # if the line is labeled 163, reads are aligned to top strand of phi80alpha-vir
        # the code will retrieve the starting and end position and will add 1 to each bp location associated with that
        if int(line[1]) == 163:
            for bp in range(int(line[3]), ((int(line[8])) + int(line[3]))):
                for_strand[bp] += 1

        # same as above, but if the line is labeled 99, reads are aligned to bottom strand of phi80alpha-vir
        elif int(line[1]) == 99:
            for bp in range(int(line[3]), ((int(line[8])) + int(line[3]))):
                for_strand[bp] += 1

        # if the line is not labeled 163 nor 99, that means it was not aligned to the genome, so skip
        else:
            pass
